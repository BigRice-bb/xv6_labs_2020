...
procinit(void)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
      ...
      // guard page.
      //
      //注释下面的代码,在初始化时不分配内核栈, 在创建进程时为每个进程分配内核栈
      // char *pa = kalloc();
      // if(pa == 0)
      //   panic("kalloc");
      // uint64 va = KSTACK((int) (p - proc));//计算进程p的内核栈的虚拟地址
...
found:
  ...
  kvmmap(p->kernelpgtbl, va, (uint64)pa, PGSIZE, PTE_R | PTE_W);//完成映射
  ...
  for(;;){
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      ...
      if(p->state == RUNNABLE) {
        ...
        p->state = RUNNING;//改变进程状态为ing
        c->proc = p;//关联当前进程 给CPU执行

        // 切换到进程独立的内核页表
        w_satp(MAKE_SATP(p->kernelpgtbl));
        sfence_vma(); // 清除快表缓存  TLB  保证地址映射关系正确
